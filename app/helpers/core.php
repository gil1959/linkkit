...
function l($key, $language = null, $null_coalesce = false, $clear_cache = false) {
    static $cached = [];
    ...
    $language_name = $language ?? \Altum\Language::$name;
    $main_name = \Altum\Language::$main_name;

    if(!isset($cached[$language_name])) {
        $cached[$language_name] = \Altum\Language::get($language_name);
    }

    if(!isset($cached[$main_name])) {
        $cached[$main_name] = \Altum\Language::get($main_name);
    }

    return $cached[$language_name][$key] ?? $cached[$main_name][$key] ?? ($null_coalesce ? null : $key);
}
