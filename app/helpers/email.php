...
function generate_unsubscribe_token($identifier, $ttl_seconds, $secret) {
...
function verify_unsubscribe_token($token, $secret) {
    $parts = explode(':', $token, 3);
    if (count($parts) !== 3) {
        return false;
    }

    [$identifier, $exp, $sig] = $parts;

    if (!ctype_digit($identifier) || !ctype_digit($exp)) {
        return false;
    }
    ...
    $msg = $identifier . ':' . $exp;
    $expected_sig = hash_hmac('sha256', $msg, $secret, true);
...
function process_send_mail_template($title, $content, $data = []) {
    ...
    $email_template = include_view(THEME_PATH . 'views/partials/email_wrapper.php', [
        ...
        'unsubscribe_url' => $data['unsubscribe_url'] ?? false,
        'content' => $content,
    ]);
