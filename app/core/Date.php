...
class Date {
    ...
    private static $timezone_cache = [];

    private static function timezone($timezone) {
        if(!$timezone) $timezone = self::$timezone ?: self::$default_timezone;

        if(!isset(self::$timezone_cache[$timezone])) {
            self::$timezone_cache[$timezone] = new \DateTimeZone($timezone);
        }

        return self::$timezone_cache[$timezone];
    }
    ...
    public static function get($date = '', $format_type = -1, $timezone = '') {
        ...
        if(is_string($date)) {
            $datetime = (new \DateTime($date))->setTimezone(self::timezone($timezone));
        } else {
            $datetime = $date->setTimezone(self::timezone($timezone));
        }
    ...
    public static function get_start_end_dates_new($start_date = null, $end_date = null, $current_timezone = null, $wanted_timezone = null) {
        ...
        if($start_date->format('d') == $end_date->format('d')) {
            ...
            $return['response_date_format'] = 'H A';
        ...
        if($days_difference >= 1) {
            ...
            $return['response_date_format'] = 'j M';
        ...
        if($months_difference >= 2) {
            ...
            $return['response_date_format'] = 'M Y';
