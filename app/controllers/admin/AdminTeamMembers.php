...
class AdminTeamMembers extends Controller {
    ...
    public function index() {
        ...
        $filters = (new \Altum\Filters(['user_id', 'team_id', 'status'], ['user_email'], ['team_member_id', 'last_datetime', 'datetime', 'user_email'], allowed_datetime_fields: ['datetime', 'last_datetime']));
        ...
        $paginator = (new \Altum\Paginator($total_rows, $filters->get_results_per_page(), $_GET['page'] ?? 1, url('admin/team-members?' . $filters->get_get() . '&page={{PAGE}}')));
        ...
        /* Export handler */
        process_export_csv_new($team_members, ['team_member_id', 'team_id', 'user_id', 'invited_email', 'name', 'access', 'status', 'datetime', 'last_datetime'], ['access'], sprintf(l('admin_team_members.title')));
