...
class AdminPaymentProcessors extends Controller {
    ...
    public function index() {
        ...
        $filters = (new \Altum\Filters(['user_id', 'payment_processor_id', 'processor', 'is_enabled'], ['name'], ['payment_processor_id', 'last_datetime', 'datetime', 'name'], allowed_datetime_fields: ['last_datetime', 'datetime']));
        ...
        $paginator = (new \Altum\Paginator($total_rows, $filters->get_results_per_page(), $_GET['page'] ?? 1, url('admin/payment-processors?' . $filters->get_get() . '&page={{PAGE}}')));
