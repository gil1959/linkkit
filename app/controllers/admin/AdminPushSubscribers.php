...
class AdminPushSubscribers extends Controller {
    ...
    public function index() {
        ...
        $filters = (new \Altum\Filters(['user_id', 'device_type', 'country_code', 'continent_code'], ['ip', 'city_name', 'os_name', 'browser_name', 'browser_language'], ['push_subscriber_id', 'datetime'], allowed_datetime_fields: ['datetime']));
        ...
        $paginator = (new \Altum\Paginator($total_rows, $filters->get_results_per_page(), $_GET['page'] ?? 1, url('admin/push-subscribers?' . $filters->get_get() . '&page={{PAGE}}')));
    ...
    public function delete() {
        ...
        if(!$internal_notification = db()->where('push_subscriber_id', $push_subscriber_id)->getOne('push_subscribers', ['push_subscriber_id', 'title'])) {
            throw_404();
        }
