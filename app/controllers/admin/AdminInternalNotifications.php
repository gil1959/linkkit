...
class AdminInternalNotifications extends Controller {
    ...
    public function index() {
        ...
        $filters = (new \Altum\Filters(['user_id', 'from_who', 'for_who', 'is_read'], ['title', 'description'], ['internal_notification_id', 'datetime', 'read_datetime'], allowed_datetime_fields: ['datetime', 'read_datetime']));
        ...
        $paginator = (new \Altum\Paginator($total_rows, $filters->get_results_per_page(), $_GET['page'] ?? 1, url('admin/internal-notifications?' . $filters->get_get() . '&page={{PAGE}}')));
