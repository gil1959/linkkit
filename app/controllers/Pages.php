...
class Pages extends Controller {
    ...
    public function index() {
        ...
        if($pages_category_url) {
            ...
            $pages_result_query = "
                SELECT `url`, `title`, `description`, `total_views`, `type`, `language`, `plans_ids`, `icon`
                FROM `pages` 
                WHERE `pages_category_id` = {$pages_category->pages_category_id} AND (`language` = '{$language}' OR `language` IS NULL) AND `type` <> 'feature' AND `is_published` = 1
            ...
            $popular_pages_result_query = "SELECT `url`, `title`, `description`, `total_views`, `type`, `language`, `plans_ids`, `icon` FROM `pages` WHERE (`language` = '{$language}' OR `language` IS NULL) AND `type` <> 'feature' AND `is_published` = 1 ORDER BY `total_views` DESC LIMIT 6";
            ...
            $pages_categories_result_query = "
                SELECT 
                    ...
                    `pages_categories`.`description`,
                    `pages_categories`.`icon`,
                    `pages_categories`.`language`,
                    COUNT(`pages`.`page_id`) AS `total_pages`
                FROM `pages_categories`
                LEFT JOIN `pages` 
                    ON `pages`.`pages_category_id` = `pages_categories`.`pages_category_id`
                    AND (`pages`.`language` = '{$language}' OR `pages`.`language` IS NULL)
                    AND `pages`.`type` <> 'feature'
                    AND `pages`.`is_published` = 1
                    AND (`pages`.`plans_ids` IS NULL OR `pages`.`plans_ids` = '' OR `pages`.`plans_ids` = '[]')
