...
class ApiTeamMembers extends Controller {
    ...
    private function post() {
        ...
        $total_rows = db()->where('team_id', $team->team_id)->getValue('teams_members', 'count(`team_member_id`)');
        ...
        $email_template = get_email_template(
            ...
            [
                ...
                '{{REGISTER_LINK}}' => url('register?redirect=teams-system&email=' . $_POST['user_email']) . '&unique_registration_identifier=' . $token,
            ],
    ...
    private function patch() {
        ...
        if($this->user->plan_settings->team_members_limit != -1 && $total_rows > $this->user->plan_settings->team_members_limit) {
            $this->response_error(sprintf(settings()->payment->is_enabled ? l('global.info_message.plan_feature_limit_removal_with_upgrade') : l('global.info_message.plan_feature_limit_removal'), $total_rows - $this->user->plan_settings->team_members_limit, mb_strtolower(l('team_members.title')), l('global.info_message.plan_upgrade')), 401);
        }
        ...
        $team_member->access = json_encode($access);
        $team_member->last_datetime = get_date();
