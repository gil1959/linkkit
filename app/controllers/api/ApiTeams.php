...
class ApiTeams extends Controller {
    ...
    private function get_all() {
        ...
        $filters = (new \Altum\Filters([], ['name'], ['team_id', 'name', 'datetime', 'last_datetime'], allowed_datetime_fields: ['datetime', 'last_datetime']));
        ...
        $paginator = (new \Altum\Paginator($total_rows, $filters->get_results_per_page(), $_GET['page'] ?? 1, url('api/teams?' . $filters->get_get() . '&page={{PAGE}}')));
    ...
    private function post() {
        ...
        /* Try to get details about the newly created resource id */
        $team = db()->where('team_id', $team_id)->where('user_id', $this->user->user_id)->getOne('teams');
    ...
    private function patch() {
        ...
        /* Check for the plan limit */
        $total_rows = db()->where('user_id', $this->user->user_id)->getValue('teams', 'count(*)');
        ...
        $team = db()->where('team_id', $team->team_id)->where('user_id', $this->user->user_id)->getOne('teams');
