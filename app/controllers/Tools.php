...
//            }
        ...
        if(!empty($_POST)) {
            ...
            if(!Alerts::has_field_errors() && !Alerts::has_errors()) {
                ...
                foreach(['mp', 'identicon', 'monsterid', 'wavatar', 'retro', 'robohash', 'blank'] as $key) {
                    $data['result'][$key] = get_gravatar($_POST['email'], 2056, $key, force_gravatar: true);
                }
    ...
    private function is_allowed_url($url) {
        ...
        if(
            $host !== 'static.resmush.it' &&
            mb_substr($host, -18) === '.static.resmush.it'
        ) {
            return true;
        }
    ...
    public function download() {
        ...
        if(!$this->is_allowed_url($_GET['url'])) {
            die();
        }
    ...
    public function view_image() {
        ...
        if(!$mime_type || mb_substr($mime_type, 0, 6) !== 'image/') {
            die();
        }
