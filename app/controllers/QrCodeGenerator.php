    ...
    public function index() {
        ...
        switch($_POST['type']) {
            ...
            case 'epc':
				...
				$epc_bic = trim((string) ($_POST['epc_bic'] ?? ''));
				$epc_payee_name = trim((string) ($_POST['epc_payee_name'] ?? ''));
				$epc_iban = trim((string) ($_POST['epc_iban'] ?? ''));
				$epc_amount = $_POST['epc_amount'] ?? null;
				$epc_remittance_reference = trim((string) ($_POST['epc_remittance_reference'] ?? ''));
				$epc_remittance_text = trim((string) ($_POST['epc_remittance_text'] ?? ''));
				$epc_information = trim((string) ($_POST['epc_information'] ?? ''));

				/* Remove line breaks so user input does not break the EPC format */
				$epc_payee_name = str_replace(["\r", "\n"], ' ', $epc_payee_name);
				$epc_remittance_reference = str_replace(["\r", "\n"], ' ', $epc_remittance_reference);
				$epc_remittance_text = str_replace(["\r", "\n"], ' ', $epc_remittance_text);
				$epc_information = str_replace(["\r", "\n"], ' ', $epc_information);

				/* Build the amount field only if a valid amount was provided */
				$amount_line = '';
				if ($epc_amount !== null && $epc_amount !== '') {
					$epc_amount = (float) $epc_amount;
					if ($epc_amount >= 0.01) {
						$amount_line = 'EUR' . number_format($epc_amount, 2, '.', '');
					}
				}

				/* Only one remittance field should be used */
				if ($epc_remittance_reference !== '' && $epc_remittance_text !== '') {
					$epc_remittance_text = '';
				}

				/* Build all EPC lines in the correct order */
				$data_lines = [
					'BCD',
					'002',
					'1',
				...
				/* Remove empty lines from the end so there is no trailing newline */
				while (count($data_lines) > 0 && end($data_lines) === '') {
					array_pop($data_lines);
				}

				/* Join all lines into the final EPC payload */
				$data = implode("\n", $data_lines);
