...
<div class="col-12 col-md-6 col-lg-4 p-3 position-relative" data-tool-id="<?= $data->tool_id ?>" data-tool-name="<?= l('tools.' . $data->tool_id . '.name') ?>" data-tool-category="<?= $data->tool_category ?? '' ?>">
    <div class="card d-flex flex-column h-100 overflow-hidden">
        <div class="card-body text-truncate">
            <div class="d-flex align-items-center justify-content-center rounded tool-icon mb-2 bg-primary-100" style="background: <?= $data->faded_color ?> !important;">
                <i class="<?= $data->tool_icon ?> fa-fw text-primary-600" style="color: <?= $data->color ?> !important;"></i>
            </div>
