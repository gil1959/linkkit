    ...
    <script type="application/ld+json">
        {
            ...
            "name": <?= json_encode(\Altum\Title::get()) ?>,
            "url": <?= json_encode(url(\Altum\Router::$original_request)) ?>,
            "applicationCategory": "WebApplication"
            <?php if($data->tools_usage[$data->tool_id]->total_ratings > 0): ?>
            ,"aggregateRating": {
                "@type": "AggregateRating",
                "ratingValue": <?= (float) $data->tools_usage[$data->tool_id]->average_rating ?>,
                "reviewCount": <?= (int) $data->tools_usage[$data->tool_id]->total_ratings ?>,
            }
            <?php endif ?>
        }
    </script>
