...
<?php if(settings()->links->static_is_enabled): ?>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <form name="create_static" method="post" role="form" enctype="multipart/form-data">
                        ...
                        <div class="form-group d-none" data-mode="ai">
                            <label for="static_input"><i class="fas fa-fw fa-sm fa-wand-magic text-muted mr-1"></i> <?= l('create_static_modal.ai_input') ?></label>
                            <textarea id="static_input" name="input" class="form-control" maxlength="<?= $this->user->plan_settings->ai_static_prompts_characters_limit == -1 ? null : $this->user->plan_settings->ai_static_prompts_characters_limit ?>" rows="5" placeholder="<?= l('create_static_modal.ai_input_placeholder') ?>"></textarea>
                        </div>

                        <div class="form-group" data-file-input-wrapper-size-limit="<?= settings()->links->static_size_limit ?>" data-file-input-wrapper-size-limit-error="<?= sprintf(l('global.error_message.file_size_limit'), settings()->links->static_size_limit) ?>" data-mode="file">
    ...
    <?php if(settings()->links->static_ai_is_enabled): ?>
        ...
        <script>
            ...
            $('#create_static').on('show.bs.modal', event => {
                let mode = $(event.relatedTarget).data('mode');

                if(mode == 'ai') {
                    $(event.currentTarget).find('button[type="submit"]').addClass('bg-animated-gradient');

                    $(event.currentTarget).find('[data-mode="ai"]').removeClass('d-none');
                    $(event.currentTarget).find('#static_input').attr('required', 'required');

                    $(event.currentTarget).find('[data-mode="file"]').addClass('d-none');
                    $(event.currentTarget).find('#static_file').removeAttr('required');
                } else {
                    $(event.currentTarget).find('button[type="submit"]').removeClass('bg-animated-gradient');

                    $(event.currentTarget).find('[data-mode="file"]').removeClass('d-none');
                    $(event.currentTarget).find('#static_file').attr('required', 'required');

                    $(event.currentTarget).find('[data-mode="ai"]').addClass('d-none');
                    $(event.currentTarget).find('#static_input').removeAttr('required');
                }
            });
        </script>
