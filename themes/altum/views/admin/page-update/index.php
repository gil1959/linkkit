    ...
    <div class="card-body">
        ...
        <form id="page_update_form" action="" method="post" role="form">
            ...
            <?php if($data->page->type == 'feature'): ?>
                <div class="form-group">
                    ...
                    <div class="row btn-group-toggle m-n2" data-toggle="buttons">
                        <div class="p-2 col-12">
                            <label class="btn btn-light btn-block font-size-small mb-0 text-truncate <?= $data->page->type == 'feature' ? 'active"' : null?>">
                                <input type="radio" name="type" value="feature" class="custom-control-input" <?= $data->page->type == 'feature' ? 'checked="checked"' : null?> required="required" />
                ...
                <?php $feature_link = \Altum\CustomHooks::generate_admin_feature_pages_settings_links()[$data->page->url] ?? null ?>
                <?php if($feature_link && $feature_link['url']): ?>
                    <div class="alert alert-info"><?= sprintf(l('admin_pages.info_message.feature'), $feature_link['url'], $feature_link['name']) ?></div>
                <?php endif ?>
                ...
                <div class="form-group">
                    ...
                    <div class="input-group">
                        ...
                        <input id="url" type="text" name="url" class="form-control <?= \Altum\Alerts::has_field_errors('url') ? 'is-invalid' : null ?>" placeholder="<?= l('global.url_slug_placeholder') ?>" value="<?= $data->page->url ?>" disabled="disabled" />
                ...
                <div class="form-group" data-position="bottom">
                    <label for="footer_category_id"><i class="fas fa-fw fa-sm fa-book text-muted mr-1"></i> <?= l('admin_pages.footer_category_id') ?></label>
                    <select id="footer_category_id" name="footer_category_id" class="custom-select">
                        <?php $footer_categories = explode(',', l('global.footer.categories')) ?>
                        <?php foreach($footer_categories as $footer_category_id): ?>
                            <option value="<?= $footer_category_id ?>" <?= $data->page->footer_category_id == $footer_category_id ? 'selected="selected"' : null ?>><?= l('global.footer.categories.' . $footer_category_id) ?></option>
                        <?php endforeach ?>
                    </select>
                </div>
                ...
                <button class="btn btn-block btn-gray-100 font-size-small font-weight-450 my-4" type="button" data-toggle="collapse" data-target="#advanced_container" aria-expanded="false" aria-controls="advanced_container">
                ...
                <div class="collapse" id="advanced_container">
                    <div class="form-group custom-control custom-switch">
                        ...
                        <label class="custom-control-label" for="open_in_new_tab"><?= l('admin_resources.open_in_new_tab') ?></label>
                    </div>
...
<script>
    ...
    let type = <?= json_encode($data->page->type) ?>;
    ...
    if(type != 'feature') {
        ...
        let editorjs = new EditorJS({
            ...
            /* Data */
            data: content ? is_valid_json(content.value) ? JSON.parse(content.value) : {} : '',
        ...
        /* Initiate codemirror */
        let codemirror_instance = CodeMirror.fromTextArea(content, {
            lineNumbers: true,
            lineWrapping: true,
            mode: 'htmlmixed',
        ...
        document.querySelector('input[name="type"]') && document.querySelectorAll('input[name="type"]').forEach(element => element.addEventListener('change', () => {
            type_handler('input[name="type"]', 'data-type');
        }));
    }

    type_handler('input[name="position"]', 'data-position');
    document.querySelector('input[name="position"]') && document.querySelectorAll('input[name="position"]').forEach(element => element.addEventListener('change', () => { type_handler('input[name="position"]', 'data-position'); }));
</script>
