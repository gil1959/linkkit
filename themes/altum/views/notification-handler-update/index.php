            ...
            <form action="" method="post" role="form">
                ...
                <div class="form-group">
                    ...
                    <div class="row btn-group-toggle m-n2" data-toggle="buttons">
                        ...
                        <?php foreach(array_keys(require APP_PATH . 'includes/notification_handlers.php') as $notification_handler): ?>
                            ...
                            if($this->user->plan_settings->{'notification_handlers_' . $notification_handler . '_limit'} != -1 && ($data->total_notification_handlers[$notification_handler] ?? 0) >= $this->user->plan_settings->{'notification_handlers_' . $notification_handler . '_limit'}) {
                                $tooltip = l('global.info_message.plan_feature_limit');
                            }
                            ?>

                            <div class="p-2 col-12 col-lg-4">
                                <label class="btn btn-light btn-block font-size-small mb-0 text-truncate <?= $data->notification_handler->type == $notification_handler ? 'active"' : null?>" data-toggle="tooltip" title="<?= $tooltip ?>">
                                    <input type="radio" name="type" value="<?= $notification_handler ?>" class="custom-control-input" <?= $data->notification_handler->type == $notification_handler ? 'checked="checked"' : null?> required="required" />
                ...
                <div class="form-group" data-type="email">
                    <div class="d-flex flex-wrap flex-row justify-content-between">
                        ...
                        <?php if($data->notification_handler->type == 'email' && !$data->notification_handler->settings->email_is_confirmed): ?>
                        <div>
                    ...
                    <small class="text-muted"><?= l('notification_handlers.email_help2') ?></small>
                </div>
                ...
                <button type="submit" name="test" class="btn btn-sm btn-block btn-outline-primary mt-3" <?= (session_get('notification_handler_test_' . ($_POST['type'] ?? '')) ?? 0) < 10 ? null : 'disabled="disabled"' ?>><?= l('notification_handlers.test') ?></button>
            </form>
...
<?php ob_start() ?>
    <script>
        ...
        let email_is_confirmed = <?= json_encode($data->notification_handler->settings->email_is_confirmed ?? true) ?>;

        let process_email_type = () => {
            let type = document.querySelector('input[name="type"]:checked').value;

            if(type == 'email' && !email_is_confirmed) {
                document.querySelector('button[name="test"]').setAttribute('disabled', 'disabled');
            } else {
                document.querySelector('button[name="test"]').removeAttribute('disabled');
            }
        }
